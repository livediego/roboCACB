...
app.post('/executar', async (req, res) => {
    ...
    try {
        ...
        const { empresas, credenciais, questionario, isProd } = req.body;

        if (!empresas || !Array.isArray(empresas) || empresas.length === 0 || !credenciais) {
            return res.status(400).json({ error: 'empresas (array) e credenciais são obrigatórios' });
        }
        ...
        for (const empresa of empresas) {
            ...
            try {
                ...
                console.log('================================================');
                console.log(`🚀 Processando empresa: ${empresa.nome_empresa}`);
                console.log('================================================');
                ...
                const duracaoEmpresa = Date.now() - inicioEmpresa;

                resultados.push({
                    empresa: empresa.nome_empresa,
                    sucesso: true,
                    duracao_ms: duracaoEmpresa
                });

                console.log(`✅ ${empresa.nome_empresa} finalizada em ${duracaoEmpresa} ms`);

                // Voltar para tela principal do indicador
                await page.goto('https://www.alinvestverde-c1-monitoreo.com/Ficha11OE?IdProyectoIndicadorML=291');
                await page.waitForLoadState('networkidle');

            } catch (erroEmpresa) {

                console.error(`❌ Erro na empresa ${empresa.nome_empresa}`, erroEmpresa);
        ...
        const duracaoTotal = fimExecucao - inicioTimestamp;
